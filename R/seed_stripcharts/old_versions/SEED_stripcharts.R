...
TABLE<-readline("Enter the path to your SEED hierarchy input table: ")
...
xlim <- c(max(abs(d$difference.between.q50)) + 0.5, - max(abs(d$difference.between.q50)) -0.5)
...
#For Jeans ALDEx output
#	cutoffb <- ((d$difference.between.q50 > 0) & (d$sig.both == TRUE))
#	cutoffn <- ((d$difference.between.q50 < 0) & (d$sig.both == TRUE))

#For B. cereus ALDEx output:
	cutoffb <- ((d$difference.between.q50 > 0) & (d$criteria.meaningful == TRUE) & (d$criteria.significant == TRUE))
	cutoffn <- ((d$difference.between.q50 < 0) & (d$criteria.meaningful == TRUE) & (d$criteria.significant == TRUE))
	nocut <- ((d$criteria.meaningful == FALSE) & (d$criteria.significant == FALSE))

	bv_sig <- unique( data.frame( group=d[[g]], absolute=d$difference.between.q50)[ cutoffb , ] )
	n_sig <- unique( data.frame( group=d[[g]], absolute=d$difference.between.q50)[ cutoffn , ] )
	no_sig <- unique( data.frame( group=d[[g]], absolute=d$difference.between.q50)[nocut,])
...
#Use add=TRUE to overplot
	...
	title(xlab="Median Absolute" ~~ Log[2] ~~ "Difference", line=2, cex.lab=0.8)
