...
xlim <- c(max(abs(d$diff.btw.q50)) + 0.5, - max(abs(d$diff.btw.q50)) -0.5)
...
#For ALDEx2 with qval
	cutoffb <- ((d$diff.btw.q50 > 0) & (d$criteria.sig.q == TRUE))
	cutoffn <- ((d$diff.btw.q50 < 0) & (d$criteria.sig.q == TRUE))
	nocut <- (d$criteria.sig.q == FALSE)

	bv_sig <- unique( data.frame( group=d[[g]], absolute=d$diff.btw.q50)[ cutoffb , ] )
	n_sig <- unique( data.frame( group=d[[g]], absolute=d$diff.btw.q50)[ cutoffn , ] )
	no_sig <- unique( data.frame( group=d[[g]], absolute=d$diff.btw.q50)[nocut,])
